...
fn main() {
    ...
    // First, we need to generate the universal parameters for the MarlinKZG10 scheme.
    let num_points = 10; // number of evaluation points
    let degree = 50;
    let rng = &mut test_rng();
    let params = PC_Bls12_381::setup(degree, None, rng).expect("Setup Failed"); // second variable "num vars" needs to be inspected.
    let (committer_key, vk) = PC_Bls12_381::trim(&params, 20, 20, None).expect("Trim Failed"); // second param, SuppoertedDegree? and supported_hiding_bound?. In the examples, SuppoertedDegree is 2.
    
    // let secret_poly = UniPoly_381::rand(10, rng);
    let secret_poly = DensePolynomial::<BlsFr>::rand(10, rng);

    let label = "my polynomial".to_string();
    let labeled_poly = LabeledPolynomial::new(label, secret_poly.clone(), None, None);
    let labeled_polynomials = vec![&labeled_poly];
    let point = rand_point::<Bls12_381>(None);

    let value = secret_poly.evaluate(&point);
    let i_value = vec![value];

    let poseidon_params = poseidon_parameters_for_test();
    let poseidon_sponge = PoseidonSponge::<BlsFr>::new(&poseidon_params);
    let mut challenge = generate_challenge(poseidon_sponge);

    let (commitments, randomness) = PC_Bls12_381::commit(&committer_key, labeled_polynomials.clone(), Some(rng)).expect("Commit failed");
    ...
    let result = PC_Bls12_381::check(&vk, &commitments, &point, i_value, &proof, &mut challenge, Some(rng)).unwrap();

    println!("Result value is: {:?}", result);
    // assert!(PC_Bls12_381::check(&vk, &commitments, &point, i_value, &proof, &mut challenge, Some(rng))?);
...
fn rand_point<E: Pairing >(_: Option<usize>) -> E::ScalarField {
    let rng = &mut test_rng();
    E::ScalarField::rand(rng)
}

fn generate_challenge<F: PrimeField, S: CryptographicSponge> (sponge: S) -> ChallengeGenerator<F, S> {
